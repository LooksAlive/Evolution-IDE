...
QWidget *FindReplaceWidget::buildButtonBox(const bool replace)
{
    m_Buttons = new QDialogButtonBox(this);
    auto find = m_Buttons->addButton(tr("Find"), QDialogButtonBox::AcceptRole);
    connect(find, &QPushButton::clicked, this, &FindReplaceWidget::handleFind);
    if (replace) {
        ...
        connect(replaceb, &QPushButton::clicked, this, &FindReplaceWidget::handleReplace);
        connect(replaceall, &QPushButton::clicked, this, &FindReplaceWidget::handleReplaceAll);
    }
    return m_Buttons;
}

QLayout *FindReplaceWidget::buildForm(const bool replace)
{
...
void FindReplaceWidget::findInTextEdit(const bool next)
{
    bool found = false;
    if (m_Edit) {
...
void FindReplaceWidget::handleFind()
{
    findInTextEdit(m_RadioDown->isChecked());
}

void FindReplaceWidget::handleReplace()
{
...
void FindReplaceWidget::handleReplaceAll()
{
    if (m_Edit && !m_Edit->isReadOnly()) {
        int i = 0;
...
void FindReplaceWidget::setTextEdit(QPlainTextEdit *edit)
{
    m_Edit = edit;
}
